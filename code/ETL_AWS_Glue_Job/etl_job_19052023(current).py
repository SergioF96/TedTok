...
tedx_dataset_agg = tedx_dataset.join(tags_dataset_agg, tedx_dataset.idx == tags_dataset_agg.idx_ref, "left") \
    ...
    .drop("num_views") 
tedx_dataset_agg=tedx_dataset_agg.where(tedx_dataset_agg.url.isNotNull())

##READ WATCH NEXT DATASET
watch_dataset_path = "s3://unibg-tecn-cloud-sfmr01/watch_next_dataset.csv"
watch_dataset = spark.read \
    .option("header",True) \
    .csv(watch_dataset_path)


#CREATE THE AGGREGATE MODEL, AGAIN, THEN ADD THE WATCH NEXT TO TEDX-TAGS DATASET
temp_aggregate= tedx_dataset_agg.where(tedx_dataset_agg.idx.isNotNull())\
    .where(tedx_dataset_agg.url.isNotNull()) \
    .select(col("idx").alias("wnext_id"),col("url").alias("wnext_url"))

watch_dataset_agg = watch_dataset.filter(~ watch_dataset['url'].rlike('watch-later'))
watch_dataset_agg = watch_dataset_agg.join(temp_aggregate, watch_dataset_agg.url==temp_aggregate.wnext_url,"inner")\
...
#DATA TRANSFER TO MONGODB DATABASE
